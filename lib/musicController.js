...
MusicController.prototype.play = function (index) {
    ...
    return spawn("sudo", ["python", this.config.basePath + "/py/synchronized_lights.py", "--file", this.config.basePath + "/mp3/" + this.config.songs[index] + '.mp3']).on('close', function (msg) {
        console.log(msg);
...
MusicController.prototype.booked = function () {
    ...
    return spawn("sudo", ["python", this.config.basePath + "/py/synchronized_lights.py", "--file", this.config.basePath + "/mp3/" + this.config.appointmentSong + '.mp3']).on('close', function (msg) {
        console.log(msg);
    }).on('error', function(err) {
        console.log(err);
    });
